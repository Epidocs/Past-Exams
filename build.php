...
function getMetaInfos($pathList)
{
	...
	foreach ($pathList as $path)
	{
		...
		if
		(!isset($metaJSONs[$dir]) AND file_exists($dir . '/meta.json'))
		{
		...
		if ($isDir)
		{
			...
			$replace = [
				...
				str_replace([' ', '#', '&'], ['_', 's', 'n'], strtolower(implode('.', $breadcrumb))),
			];
...
function buildSubFolders($folders)
{
	...
	foreach($folders as $folder)
	{
		...
		// Make new folders
		$lowercaseFolder = str_replace([' ', '#', '&'], ['_', 's', 'n'], strtolower($folder));
